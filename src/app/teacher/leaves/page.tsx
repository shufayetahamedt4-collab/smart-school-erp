...
import { Card, CardHeader, Badge, Field, TextInput, Textarea, PageHeader, LoadingScreen, EmptyState, ErrorNote, statusTone, prettyStatus } from "@/components/ui";
...
export default function TeacherLeavesPage() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="My Leaves" subtitle="Apply for leave — admin approves (PRD §9.2); approved leave triggers substitution suggestions" />
      ...
      <Card className="p-4">
        ...
        <div className="grid grid-cols-1 items-end gap-3 sm:grid-cols-4">
          ...
          <Field label="Reason *" className="sm:col-span-1"><TextInput value={form.reason || ""} onChange={(e) => setForm({ ...form, reason: e.target.value })} /></Field>
          <button className="btn btn-primary" onClick={apply} disabled={busy || !form.fromDate || !form.reason}><Plus size={15} /> Apply</button>
        </div>
      </Card>
